...
from src.node_streaming.abi import UNISWAP_V2_FACTORY
from src.node_streaming.web3_pool import Web3Pool


class UniswapV2PoolFinder():
    ...
    def find_pools(self, token_address: str) -> list[ContractEventDefinition]:
        ...
        factory_contract = self.w3.web3.eth.contract(address=uniswap_factory_address, abi=UNISWAP_V2_FACTORY)

        # Filter for the PairCreated event where one of the tokens is WETH
        # Define the block range to query
        block = 0  # Adjust as needed
        end_block = self.w3.web3.eth.block_number
        temp_block = end_block

        # Iterate through block ranges
        pairs = []
        while block < end_block:
            pair_created_filter = factory_contract.events.PairCreated.create_filter(
                fromBlock=block,
                toBlock=min(temp_block, end_block),
                argument_filters={'token0': token_address}  # And/or token1
            )
            try:
                new_pairs = [event['args']['pair'] for event in pair_created_filter.get_all_entries()]
            except ValueError as e:
                temp_block = int(e.args[0]['message'].split("[")[1].split(",")[1][1:-1],16)
                continue
            print((new_pairs))
